...
その複雑さ解消を目的に、本記事ではNext.jsとmicroCMSを組み合わせたプレビュー環境の実装例を紹介します。
...
といった流れです。（そもそもCSRなどレンダリング手法があやふやな方は[コチラ](https://www.patterns.dev/posts/rendering-introduction/)や[コチラ](https://zenn.dev/akino/articles/78479998efef55)を参照ください）  
...
### 秘匿化
API-keyを露出させたくないケースもあると思います。
その際はサーバレス関数(Lamda, Next.js API Routes など)の利用やSSRを検討しましょう。
...
API-keyを露出させたくないというケースですので、`NEXT_PUBLIC_`を使わずサーバーで利用するよう設定しましょう。


:::message
上記のような対策をとったとしても、そもそものdraftKeyを含むURLが漏洩すると第三者のアクセスは可能になります更なるセキュリティ万全を施す場合は環境を分ける、Basic認証を付与するなどの施策を追加しましょう。
またdraftKeyはmicroCMSの画面から容易に変更可能です。漏洩した場合の対策について、運営側あらかじめ対応を決めておくと良いかもしれません。
:::

## まとめ
今回はNext.jsとmicroCMSを用いたプレビュー実装方法とその留意点をまとめました。  
ホスティングサービスからもさまざまなプレビュー機能が提供されていますが、ぜひまずはこちらで紹介した方法で実装を試してみてください。  
